...
def computeSNR(inputFile, window, M, N, H):
    ...
    Senyal_1 = np.sum(x**2) # Full Signal
    Soroll_1 = np.sum(e**2)
    Senyal_2 = np.sum(x[M:x.size-M]**2)	# Cutted Signal
    Soroll_2 = np.sum(e[M:e.size-M]**2)
	
    SNR_1 = 10*np.log10(Senyal_1/Soroll_1) #SNR1 Calculation
    SNR_2 = 10*np.log10(Senyal_2/Soroll_2) #SNR2 Calculation
